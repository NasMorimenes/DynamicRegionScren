; Classe para escanear imagem/pixels por Spawnova (15/05/22)
; https://github.com/Spawnova/ShinsImageScanClass
;
; Classe de uso geral para escanear dados de imagem para imagens e pixels, para ahk V2
;
; A transparência da imagem não é tratada nesta classe; se uma imagem contiver pixels que não são totalmente opacos, eles simplesmente serão ignorados;
; o que pode ser usado para corresponder imagens com partes que devem ser ignoradas, tendo esses pixels menos que 0xFF000000
;
; Projetei esta classe para ser relativamente fácil de usar, mas ainda manter um bom desempenho.
; Não posso prometer que funcionará para todas as configurações, só posso confirmar que funciona para mim no Windows 10 (32 e 64 bits)
;
; Todas as posições de coordenadas retornadas dependem do modo usado: desktop usa espaço de tela, cliente usa espaço do cliente, caso contrário, espaço da janela.
;
; Usar o modo desktop com o Windows Aero habilitado é muito mais lento, tente usar o modo janela/cliente sempre que possível.

;#Requires AutoHotkey v2+

	
	; UseClientArea_OrMainMonitor: 
	; 
	; OrMainMonitor = Escaneie apenas o monitor principal, se falso, ele escaneará todo o espaço da área de trabalho virtual (multi-monitores)
	
		; quando desativado, requer que você chame manualmente Update() para atualizar os dados de pixel, útil quando você precisa escanear várias coisas em um único quadro
		
		; quando ativado, tenta usar ControlClick para enviar cliques, o que funciona para programas em segundo plano
		; nem todos os programas responderão a isso, então pode ser necessário usar cliques normais que precisam estar em primeiro plano
		
		; se o Windows tiver definido a escala da área de trabalho para algo diferente de 100%, você pode ajustar com esta variável, por exemplo, se a escala do Windows estiver definida para 150%, defina esta variável para 1,5
		
		; Configuração interna
		
		; enumera o callback dos monitores
		
		; Define o modo de coordenadas para o mouse
		
		; Define o modo de coordenadas para o mouse
		
		; tenta usar ControlClick para clicar
		
		; Define a escala padrão para 1.0
		
		; carrega a biblioteca gdiplus
		
		; lida com 32 e 64 bits
		
		; lida com a varredura de toda a área de trabalho
		
		; lida com o uso da área do cliente
		
		; define o modo de coordenadas para o espaço do cliente
		
		; define o modo de coordenadas para o espaço da janela
		
		; inicializa os buffers
		
		; código de montagem para escanear imagem

#Requires AutoHotkey v2+

class ShinsImageScanClass {

	; título: 									título da janela ahk ou outro tipo de identificador, deixe em branco ou defina como 0 para escanear toda a área de trabalho
	;UseClientArea_OrMainMonitor		:		UseClientArea = Se uma janela for especificada, ela usará a área do cliente (geralmente não inclui a barra de título e menus)
	;											Caso contrário, incluirá toda a janela, o que também inclui espaço extra nas laterais e na parte inferior usado para arrastar com o mouse.
	;											Escaneie apenas o monitor principal, se falso, ele escaneará todo o espaço da área de trabalho virtual (multi-monitores)
	
	__New(title:=0, UseClientArea_OrMainMonitor:=1) {
		global ShinsImageScanClass_Monitors
	
		this.AutoUpdate 		:= 1 	;quando desativado, requer que você chame manualmente Update() para atualizar os dados de pixel, útil quando você precisa escanear várias coisas em um único quadro

		this.UseControlClick 	:= 0  	;quando ativado, tenta usar ControlClick para enviar cliques, o que funciona para programas em segundo plano
										;nem todos os programas responderão a isso, então pode ser necessário usar cliques normais que precisam estar em primeiro plano
										
		this.WindowScale 		:= 1	;se o Windows tiver definido a escala da área de trabalho para algo diferente de 100%, você pode ajustar com esta variável, por exemplo, se a escala do Windows estiver definida para 150%, defina esta variável para 1,5
	...
	;Imagem
	;
	;image				:				Caminho para o arquivo de imagem
	;variance			:				Valor entre 0-255, determina quão próximos/distantes os pixels devem estar para corresponder à cor alvo
	;&returnX			:				Variável para armazenar o resultado x
	;&returnY			:				Variável para armazenar o resultado y
	;centerResults		:				Retornar os resultados centralizados na imagem
	;scanDir			:				Direção da varredura, padrão = LRTB (varrer da esquerda para a direita, de cima para baixo)
	;
	;return				;				Retorna 1 se a imagem foi encontrada; 0 caso contrário
	...
	;
	;image				:				Caminho para o arquivo de imagem
	;x1					:				Posição x inicial no canto superior esquerdo
	;y1					:				Posição y inicial no canto superior esquerdo
	;w					:				Largura de pixels a serem pesquisados, começando de x1
	;h					:				Altura de pixels a serem pesquisados, começando de y1
	;variance			:				Valor entre 0-255, determina quão próximos/distantes os pixels devem estar para corresponder à cor alvo
	;&returnX			:				Variável para armazenar o resultado x
	;&returnY			:				Variável para armazenar o resultado y
	;centerResults		:				Retornar os resultados centralizados na imagem
	;scanDir			:				Direção da varredura, padrão = LRTB (varrer da esquerda para a direita, de cima para baixo)
	;
	;return				;				Retorna 1 se a imagem foi encontrada na região especificada; 0 caso contrário
...
;variance			:				Valor entre 0-255, determina quão próximos/distantes os pixels devem estar para corresponder à cor alvo
;
;return				;				Retorna a quantidade de imagens encontradas; 0 caso contrário
	...
	;####################################################################################################################################################################################################################################
	;Contagem de Imagens na Região
	;
	;image				:				Caminho para o arquivo de imagem
	;x1					:				Posição x inicial superior esquerda
	;y1					:				Posição y inicial superior esquerda
	;w					:				Largura de pixels a serem pesquisados, começando de x1
	;h					:				Altura de pixels a serem pesquisados, começando de y1
	;variance			:				Valor entre 0-255, determina quão próximos/distantes os pixels devem estar para corresponder à cor alvo
	;
	;return				;				Retorna a quantidade de imagens encontradas na região especificada; 0 caso contrário
	...
	;####################################################################################################################################################################################################################################
	;Imagem Mais Próxima do Ponto
	;
	;image				:				Caminho para o arquivo de imagem
	;pointX				:				Posição x do ponto
	;pointY				:				Posição y do ponto
	;variance			:				Valor entre 0-255, determina quão próximos/distantes os pixels devem estar para corresponder à cor alvo
	;&returnX			:				Variável para armazenar o resultado x
	;&returnY			:				Variável para armazenar o resultado y
	;centerResults		:				Valor entre 0-1, se habilitado, as posições retornadas serão centralizadas em vez do canto superior esquerdo da imagem alvo
	;									Isto também afeta os cálculos de distância para a imagem mais próxima
	;MaxRadius			:				Raio circular máximo para pesquisa; valores menores exigem que as imagens estejam mais próximas do ponto
	;
	;return				;				Retorna 1 se uma imagem foi encontrada perto o suficiente do ponto; 0 caso contrário
	...
	;
	;image				:				Caminho para o arquivo de imagem
	;&array				:				Um array que conterá todas as localizações das imagens (o primeiro elemento seria array[1].x e array[1].y)
	;variance			:				Valor entre 0-255, determina quão próximos/distantes os pixels devem estar para corresponder à cor alvo
	;centerResults		:				Valor entre 0-1, se habilitado, as posições retornadas serão centralizadas em vez do canto superior esquerdo da imagem alvo
	;
	;return				;				Retorna 1 (e atualiza &array) se qualquer número de imagens foram encontradas; 0 caso contrário
...
;####################################################################################################################################################################################################################################
;Array de Imagens na Região
;
;image				:				Caminho para o arquivo de imagem
;&array				:				Um array que conterá todas as localizações das imagens (o primeiro elemento seria array[1].x e array[1].y)
;x1					:				Posição x inicial no canto superior esquerdo
;y1					:				Posição y inicial no canto superior esquerdo
;w					:				Largura dos pixels a serem pesquisados, começando de x1
;h					:				Altura dos pixels a serem pesquisados, começando de y1
;variance			:				Valor entre 0-255, determina quão próximos/distantes os pixels devem estar para corresponder à cor alvo
;centerResults		:				Valor entre 0-1, se habilitado, as posições retornadas serão centralizadas em vez do canto superior esquerdo da imagem alvo
;
;return				;				Retorna 1 (e atualiza &array) se qualquer número de imagens foram encontradas na região especificada; 0 caso contrário
	...
	;Pixel
	;
	;color				:				Cor do pixel a ser encontrado (pode estar no formato 0xRRGGBB ou 0xAARRGGBB; alpha é ignorado)
	;variance			:				Valor entre 0-255, determina quão próximos/distantes os pixels devem estar para corresponder à cor alvo
	;&returnX			:				Variável para armazenar o resultado x
	;&returnY			:				Variável para armazenar o resultado y
	;scanDir			:				Direção de varredura, padrão = LRTB (varre da esquerda para a direita, de cima para baixo)
	;
	;return				;				Retorna 1 se o pixel foi encontrado; 0 caso contrário
	...
	;
	;color				:				Cor do pixel a ser encontrado (pode estar no formato 0xRRGGBB ou 0xAARRGGBB; alpha é ignorado)
	;x1					:				Posição inicial x do canto superior esquerdo
	;y1					:				Posição inicial y do canto superior esquerdo
	;w					:				Largura em pixels para pesquisar, começando de x1
	;h					:				Altura em pixels para pesquisar, começando de y1
	;variance			:				Valor entre 0-255, determina quão próximos/distantes os pixels devem estar para corresponder à cor alvo
	;&returnX			:				Variável para armazenar o resultado x
	;&returnY			:				Variável para armazenar o resultado y
	;scanDir			:				Direção de varredura, padrão = LRTB (varre da esquerda para a direita, de cima para baixo)
	;
	;return				;				Retorna 1 se um pixel dentro da região especificada foi encontrado; 0 caso contrário
	...
	;PixelPosition
	;
	;color				:				Cor do pixel para corresponder em uma determinada posição (pode estar no formato 0xRRGGBB ou 0xAARRGGBB; alpha é ignorado)
	;pointX				:				Posição X
	;pointY				:				Posição Y
	;variance			:				Valor entre 0-255, determina quão próximos/distantes os pixels devem estar para corresponder à cor alvo
	;
	;return				;				Retorna 1 se a cor corresponder na posição especificada; 0 caso contrário
	...
	;PixelCount
	;
	;color				:				Cor do pixel a ser encontrada (pode estar no formato 0xRRGGBB ou 0xAARRGGBB; alpha é ignorado)
	;variance			:				Valor entre 0-255, determina quão próximos/distantes os pixels devem estar para corresponder à cor alvo
	;
	;return				;				Retorna a quantidade de pixels correspondentes; 0 caso contrário
	...
	;variance			:				Valor entre 0-255, determina quão próximos/distantes os pixels devem estar para corresponder à cor alvo
	;
	;return				;				Retorna a quantidade de pixels correspondentes na região especificada; 0 caso contrário
	...
	;pointX				:				Posição X
	;pointY				:				Posição Y
	;radius				:				Raio para pesquisar
	;variance			:				Valor entre 0-255, determina quão próximos/distantes os pixels devem estar para corresponder à cor alvo
	;
	;return				;				Retorna a quantidade de pixels correspondentes em um raio especificado; 0 caso contrário
	...
	;h					:				Altura em pixels para pesquisar, começando de y1
	;variance			:				Valor entre 0-255, determina quão próximos/distantes os pixels devem estar para corresponder à cor alvo
	;maxResults			:				Quantidade máxima de resultados a encontrar; qualquer valor acima de 250k será ignorado
	;
	;return				;				Retorna a quantidade de pixels correspondentes na região especificada; 0 caso contrário
	...
	;pointX				:				Posição X
	;pointY				:				Posição Y
	;suppressWarning    :               Habilitar/desabilitar mensagem de aviso ao escanear fora da região
	;
	;return				;				Retorna o pixel na localização pointX, pointY
	...
	;SaveImage
	;
	;name				:				Nome para salvar o arquivo
	;x					:				Posição X da região
	;y					:				Posição Y da região
	;w					:				Largura da região, padrão é a imagem inteira
	;h					:				Altura da região, padrão é a imagem inteira
	;
	;notes				;				Salva o buffer de pixels atual em uma imagem png
	...
	;Click
	;
	;pointX				:				Posição X para clicar
	;pointY				:				Posição Y para clicar
	;button				:				Tipo de clique (esquerdo, direito, meio)
	;
	;return				;				Retorna 1 em caso de sucesso; 0 caso contrário
	;
	;notes				:				ControlClick não funcionará para todas as aplicações
	...
	;ClickDrag
	;
	;pointX1			:				Posição X para iniciar o clique
	;pointY1			:				Posição Y para iniciar o clique
	;pointX2			:				Posição X para terminar o clique
	;pointY2			:				Posição Y para terminar o clique
	;button				:				Tipo de clique (esquerdo, direito, meio)
	;
	;return				;				Retorna 1 em caso de sucesso; 0 caso contrário
	;
	...
	;return				;				Retorna 1 em caso de sucesso; 0 caso contrário
	;
	;notes				:				Clica aleatoriamente dentro da região especificada
	...
	;&w					:				Variável para armazenar a largura resultante
	;&h					:				Variável para armazenar a altura resultante
	;
	;return				;				Retorna void
	;
	;notes				:			 Obtém a largura/altura de uma imagem em cache
