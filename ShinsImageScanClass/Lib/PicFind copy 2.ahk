...
C
img1 = { #FF0000, #00FF00, #0000FF, #FFFF00, #FF00FF, #00FFFF, #000000, #FFFFFF, #808080 }
...
C
img1 = { #0000FF, #0000FF #0000FF, #FFFF00, #0000FF, #FFFF00, #0000FF, #0000FF, #0000FF }

Ver Excel

Se "#0000FF	(Azul)" for definida como cor de interese

binImg = { 1, 1, 1, 0, 1, 0, 1, 1, 1}

FFF
*/
;Interação sem Stride
img1 := text := [ 0x0000FF, 0x0000FF 0x0000FF, 0xFFFF00, 0x0000FF, 0xFFFF00, 0x0000FF, 0x0000FF, 0x0000FF ]
;bimImg := text := [ 1, 1, 1, 0, 1, 0, 1, 1, 1 ]
hImg := h := 3
wImg := w := 3
CountIndex := 0
CountLine := 0
Stride := GetStride( wImg, 4 )
mode := 4
new_h := hImg
new_w := wImg
sw := wImg
...
loop hImg {
	...
	loop wImg {		
		++CountIndex
		Index := "( " CountLine - 1 ", " A_Index - 1 " ) := " bimImg[ CountIndex ]
		if ( mode == 4 ) {

			i := ( ( CountLine - 1 ) * new_h // wImg ) * Stride + ( ( A_Index - 1 ) * new_w // wImg ) *4 
		}
		else {
			i := ( ( CountLine - 1 ) * new_h // wImg ) * sw + ( ( A_Index - 1 ) * new_w // wImg )
		}
...
if ( mode == 5 ) {
	...
	if  ( k := ( color != 0 ) ) { ;FindPic
		;cors = ( unsigned int * ) ( text + w * h * 4 ) ;
		r := ( c >> 16 ) & 0xFF
		g := ( c >> 8 ) & 0xFF
		b := c & 0xFF
		;		MsgBox( "Red -> " r "`nGreen -> " g "`nBlue -> " b )
		dR := r * r
		dG := g * g
		dB := b * b
...
int FindSingleColor(
    unsigned char *text, unsigned int *cors, int w, int h, int new_w, int new_h,
    int Stride, unsigned int c, int dR, int dG, int dB, unsigned int *s1, unsigned int *s0, int *len1)
{
    int x, y, o = 0, rr, gg, bb, r, g, b;
    *len1 = 0;
...
FindText( x1, y1, x2, y2, err1, err0, text, ScreenShot:=1, FindAll:=1, JoinText:=0, offsetX:=20, offsetY:=10 ) {        ;  FindText - Capture screen image into text and then find it
  ...
  XXX**** C source code of machine code *****
